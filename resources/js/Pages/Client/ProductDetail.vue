      ...
      <div class="col-md-6">
        <div class="text-center">
          <img
            width="60%"
            class="img-fluid"
            :src="`${url_image}${product.img_thumbnail}`"
      ...
      <div v-if="product?.category?.name" class="col-md-6">
        ...
        <div v-if="colors.length > 0" class="row_color my-2">
          ...
          <template v-if="colors" v-for="(color, index) in colors" :key="index">
            <button
              :class="{ active: activeColor == color.color.id }"
              @click="handleActiveColor(color.color.id)"
              class="button_select"
            >
              {{ color.color.name }}
        ...
        <div v-if="sizes.length > 0" class="row_size">
          ...
          >
            <button
              :class="{
                active: activeSize == variant.size.id,
                'disabled-button': variant.quantity <= 0,
              }"
              @click="handleActiveSize(variant.size.id)"
              class="button_select mx-1"
            >
        ...
        <div class="group_button_buy row row-cols-3 my-3 gap-2">
          <button type="submit" class="button_buynow col btn btn-danger">
            <i class="fa-solid fa-bag-shopping"></i> Mua ngay
          </button>
          <button @click="handleAddCart" class="button_add_cart col btn btn-warning">
...
const setDefaultSize = () => {
  const seen = new Set();
  sizes.value = product.value.variants?.filter((variant) => {
...
const showPrice = () => {
  if (activeColor.value != null && activeSize.value != null) {
    const variant = product.value.variants.find(
      (variant) =>
        variant.color_id == activeColor.value && variant.size_id == activeSize.value
    );
...
const handleActiveColor = (color_id) => {
  if (color_id == activeColor.value) {
    ...
    sizes.value = product.value.variants.filter(
      (variant) => variant.color_id === color_id
    );
    showPrice();
  }
};
